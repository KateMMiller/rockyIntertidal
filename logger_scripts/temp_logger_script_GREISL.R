...
# within +/- 2 hours of high tide for GREISL
...
filename = "GREISL_TEMP2_2017_10404527.csv"
#list.files(path)
GRE17 <- read.table(paste0(path, filename), skip = 1, sep = ",", header = T)[1:3]
colnames(GRE17) <- c("row", "timestamp", "Degrees_C")
GRE17$timestamp <- as.POSIXct(GRE17$timestamp,
                              format = "%m/%d/%y %I:%M:%S %p",
                              tz = "America/New_York") #Sys.timezone() also works
GRE17$timestamp_temp <- GRE17$timestamp
GRE17$Degrees_F <- GRE17$Degrees_C * (9/5) + 32
...
GREISL <- rbind(
  prep_tempdata(path = "../data/rocky/temp_data/collected_2011/",
                filename = temp11[grepl("GREISL.TEMP1", temp11, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2012/",
  #                filename = temp12[grepl("GREISL.TEMP1", temp12, ignore.case = TRUE)]), # No 2012 data
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2013/",
  #               filename = temp13[grepl("GREISL.TEMP1", temp13, ignore.case = TRUE)]), # No 2013 data
  prep_tempdata(path = "../data/rocky/temp_data/collected_2014/",
                filename = temp14[grepl("GREISL.Temp1", temp14, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2015/",
                filename = temp15[grepl("GREISL.TEMP2", temp15, ignore.case = TRUE)]), # T1 missing
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2016/",
  #               filename = temp16[grepl("GREISL.TEMP1", temp16, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2017/",
  #               filename = temp17[grepl("GREISL.TEMP2", temp17, ignore.case = TRUE)]), #T1&2 missing
  GRE17 |> select(row, timestamp, Degrees_F, timestamp_temp),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2018/",
                filename = temp18[grepl("GREISL.TEMP1", temp18, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2019/",
                filename = temp19[grepl("GREISL.TEMP1", temp19, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2020/",
  #               filename = temp20[grepl("GREISL.TEMP1", temp20, ignore.case = TRUE)]), # No 2020 data
  prep_tempdata(path = "../data/rocky/temp_data/collected_2021/",
                filename = temp21[grepl("GREISL.TEMP1", temp21, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2022/",
                filename = temp22[grepl("GREISL.TEMP1", temp22, ignore.case = TRUE)])
  )
...
ht_temp <- high_tide[GREISL, on = 'timestamp', roll = "nearest"]
...
ggplot(ht_temp, aes(x = timestamp_temp, y = Degrees_F)) +
  ...
  labs(x = NULL, y = "High Tide Water Temp (F) GREISL T1")+
...
ggsave("./testing_scripts/GREISL_T1_high_tide_water_level_F.jpg")

tpath = "../data/rocky/temp_data/Compiled_HT_water_temps/"
write.csv(ht_temp, paste0(tpath, "GREISL_T1-3_2011-2022.csv"), row.names = F)
