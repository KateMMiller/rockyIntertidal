...
# within +/- 2 hours of high tide for SCHPOI
...
# Fix dates and Temp units for SCHPOI_T1_2017
path = "../data/rocky/temp_data/collected_2016/"
list.files(path)
filename = "SCHPOI_TEMP1_20160624_10008627.csv"

SCH16 <- read.table(paste0(path, filename), skip = 1, sep = ",", header = T)[1:3]
colnames(SCH16) <- c("row", "timestamp_orig", "Degrees_F")
SCH16$timestamp_orig <- as.POSIXct(SCH16$timestamp_orig,
                                   format = "%m/%d/%y %I:%M:%S %p",
                                   tz = "America/New_York") #Sys.timezone() also works

SCH16$timestamp <- SCH16$timestamp_orig - lubridate::years(26) -
  lubridate::days(53) -
  lubridate::hours(12)

SCH16$timestamp_temp <- SCH16$timestamp

# Still missing 2016 fall to 2017 spring
head(SCH16)
tail(SCH16)
#SCH16$Degrees_F <- SCH16$Degrees_C * (9/5) + 32
...
SCHPOI <- rbind(
  prep_tempdata(path = "../data/rocky/temp_data/collected_2011/",
                filename = 'SCHPOI_TEMP1_2011_2038496.csv'),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2012/",
                 filename = temp12[grepl("SCHPOI.TEMP1", temp12, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2013/",
  #               filename = temp13[grepl("SCHPOI.TEMP1", temp13, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2014/",
                filename = temp14[grepl("SCHPOI.TEMP1", temp14, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2015/",
                filename = temp15[grepl("SCHPOI.TEMP1", temp15, ignore.case = TRUE)]),
  SCH16 |> select(row, timestamp, Degrees_F, timestamp_temp),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2016/",
  #               filename = temp16[grepl("SCHPOI.TEMP1", temp16, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2017/",
  #               filename = temp17[grepl("SCHPOI.TEMP1", temp17, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2018/",
                filename = temp18[grepl("SCHPOI.TEMP1", temp18, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2019/",
                filename = temp19[grepl("SCHPOI.TEMP1", temp19, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2020/",
                filename = temp20[grepl("SCHPOI.TEMP1", temp20, ignore.case = TRUE)]),
  # SCH20 |> select(row, timestamp, Degrees_F, timestamp_temp),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2021/",
                filename = temp21[grepl("SCHPOI.TEMP1", temp21, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2022/",
                filename = temp22[grepl("SCHPOI.TEMP1", temp22, ignore.case = TRUE)])
  )
...
ht_temp <- high_tide[SCHPOI, on = 'timestamp', roll = "nearest"]
...
ggplot(ht_temp, aes(x = timestamp_temp, y = Degrees_F)) +
  ...
  labs(x = NULL, y = "High Tide Water Temp (F) SCHPOI T1")+
...
ggsave("./testing_scripts/SCHPOI_T1_high_tide_water_level_F.jpg")

tpath = "../data/rocky/temp_data/Compiled_HT_water_temps/"
write.csv(ht_temp, paste0(tpath, "SCHPOI_T1-3_2011-2022.csv"), row.names = F)
