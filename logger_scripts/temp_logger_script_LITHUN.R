...
# Fixing 2019-2020
path = "../data/rocky/temp_data/collected_2020/"
filename = "LITHUN_TEMP1_10996540_Nov2020.csv"

# list.files(path)
LIT20 <- read.table(paste0(path, filename), skip = 1, sep = ",", header = T)[1:3]
colnames(LIT20) <- c("row", "timestamp", "Degrees_F")

LIT20$timestamp <- as.POSIXct(LIT20$timestamp,
                              format = "%m/%d/%y %I:%M:%S %p",
                              tz = "America/New_York") #Sys.timezone() also works
head(LIT20)

LIT20$timestamp_temp <- LIT20$timestamp
...
LITHUN <- rbind(
  prep_tempdata(path = "../data/rocky/temp_data/collected_2011/",
                filename = temp11[grepl("LITHUN.TEMP1", temp11, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2012/",
  #                filename = temp12[grepl("LITHUN.TEMP1", temp12, ignore.case = TRUE)]), # No 2012 data
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2013/",
  #               filename = temp13[grepl("LITHUN.TEMP1", temp13, ignore.case = TRUE)]), # No 2013 data
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2014/",
  #               filename = temp14[grepl("LITHUN.TEMP1", temp14, ignore.case = TRUE)]), # No 2014 data
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2015/",
  #               filename = temp15[grepl("LITHUN.TEMP1", temp15, ignore.case = TRUE)]), # No 2015 data
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2016/",
  #               filename = temp16[grepl("LITHUN.TEMP1", temp16, ignore.case = TRUE)]), # No 2016 data
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2017/",
  #               filename = temp17[grepl("LITHUN.TEMP1", temp17, ignore.case = TRUE)]), # No 2017 data
  prep_tempdata(path = "../data/rocky/temp_data/collected_2018/",
                filename = temp18[grepl("LITHUN.TEMP1", temp18, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2019/",
                filename = temp19[grepl("LITHUN.TEMP1", temp19, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2020/",
  #               filename = temp20[grepl("LITHUN.TEMP1", temp20, ignore.case = TRUE)]), # No 2020 data
  LIT20 |> select(row, timestamp, Degrees_F, timestamp_temp),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2021/",
                filename = temp21[grepl("LITHUN.TEMP1", temp21, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2022/",
                filename = temp22[grepl("LITHUN.TEMP1", temp22, ignore.case = TRUE)])
  )
...
ht_temp <- high_tide[LITHUN, on = 'timestamp', roll = "nearest"]
...
ggsave("./testing_scripts/LITHUN_T1_high_tide_water_level_F.jpg")

tpath = "../data/rocky/temp_data/Compiled_HT_water_temps/"
write.csv(ht_temp, paste0(tpath, "LITHUN_T1-3_2011-2022.csv"), row.names = F)
