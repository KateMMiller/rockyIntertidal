...
# within +/- 2 hours of high tide for OTTPOI
...
filename = "OTTPOI_TEMP1_10996540_Nov2020.csv"

# list.files(path)
OTT20 <- read.table(paste0(path, filename), skip = 1, sep = ",", header = T)[1:3]
colnames(OTT20) <- c("row", "timestamp", "Degrees_F")

OTT20$timestamp <- as.POSIXct(OTT20$timestamp,
                              format = "%m/%d/%y %I:%M:%S %p",
                              tz = "America/New_York") #Sys.timezone() also works
head(OTT20)

OTT20$timestamp_temp <- OTT20$timestamp
...
OTTPOI <- rbind(
  prep_tempdata(path = "../data/rocky/temp_data/collected_2011/",
                filename = temp11[grepl("OTTPOI.TEMP1", temp11, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2012/",
                 filename = temp12[grepl("OTTPOI.TEMP1", temp12, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2013/",
  #               filename = temp13[grepl("OTTPOI.TEMP1", temp13, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2014/",
                filename = temp14[grepl("OTTPOI.TEMP1", temp14, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2015/",
  #               filename = temp15[grepl("OTTPOI.TEMP1", temp15, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2016/",
  #               filename = temp16[grepl("OTTPOI.TEMP1", temp16, ignore.case = TRUE)]),
  # prep_tempdata(path = "../data/rocky/temp_data/collected_2017/",
  #               filename = temp17[grepl("OTTPOI.TEMP1", temp17, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2018/",
                filename = temp18[grepl("OTTPOI.TEMP1", temp18, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2019/",
                filename = temp19[grepl("OTTPOI.TEMP1", temp19, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2020/",
                filename = temp20[grepl("OTTPOI.TEMP1", temp20, ignore.case = TRUE)]),
  # OTT20 |> select(row, timestamp, Degrees_F, timestamp_temp),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2021/",
                filename = temp21[grepl("OTTPOI.TEMP1", temp21, ignore.case = TRUE)]),
  prep_tempdata(path = "../data/rocky/temp_data/collected_2022/",
                filename = temp22[grepl("OTTPOI.TEMP1", temp22, ignore.case = TRUE)])
  )
...
ht_temp <- high_tide[OTTPOI, on = 'timestamp', roll = "nearest"]
...
ggplot(ht_temp, aes(x = timestamp_temp, y = Degrees_F)) +
  ...
  labs(x = NULL, y = "High Tide Water Temp (F) OTTPOI T1")+
...
ggsave("./testing_scripts/OTTPOI_T1_high_tide_water_level_F.jpg")

tpath = "../data/rocky/temp_data/Compiled_HT_water_temps/"
write.csv(ht_temp, paste0(tpath, "OTTPOI_T1-3_2011-2022.csv"), row.names = F)
